...
def main(args, config):
    ...
    if args.mode == 'train' and args.resume_training:
        ...
        #     logger(f"Scheduler state after loading checkpoint: {scheduler.state_dict()}\n")

        # for i in range(start_epoch):
        #     if scheduler_type == "epoch": scheduler.step()
        #     elif scheduler_type == "validation": scheduler.step(best_wer)

        # if scheduler is not None:
        #     print(f"Scheduler state after loading checkpoint: {scheduler.state_dict()}\n")
        # ========================================

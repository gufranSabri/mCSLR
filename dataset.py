...
class CombinedDataset(Dataset.Dataset):
    ...
    def prepare_dataset(self, data_path):
        self.list, self.dataset_map, self.data, self.rgb_fts = [], [], {}, {}
        self.gloss_embs = {}
        for dataset in self.datasets:
            path = f"{data_path}/{dataset}/pose.{self.phase}"
            rgb_path = f"{data_path}/{dataset}/ustm_prelogits/{self.phase}"

            with open(path, "rb") as f:
                loaded_object = pickle.load(f)
                self.rgb_fts.update({key:os.path.join(rgb_path, key.replace(self.phase+"/", ""))+".npy" for key in loaded_object.keys()})

                keys_to_remove = []
                for key in self.rgb_fts.keys():
                    keys_to_remove.append(key) if not os.path.exists(self.rgb_fts[key]) else None
                for key in keys_to_remove:
                    del self.rgb_fts[key]

                temp_list = [key for key, value in loaded_object.items() if key in self.rgb_fts]
    ...
    def __getitem__(self, index):
        ...
        return name_sample, keypoint, rgb_ft, gloss, gloss_emb, length, dataset
    
    def collate_fn(self, batch):
        tgt_batch, keypoint_batch, rgb_ft_batch, src_length_batch, name_batch, datasets, gloss_emb_batch, rgb_lgt = [], [], [], [], [], [], [], []

        for name_sample, keypoint_sample, rgb_ft_sample, tgt_sample, gloss_emb, length, dataset in batch:
            ...
            rgb_ft_batch.append(rgb_ft_sample.squeeze(0))
            rgb_lgt.append(rgb_ft_sample.shape[1])
        ...
        src_input['rgb_ft'] = pad_sequence(rgb_ft_batch, batch_first=True, padding_value=0.0)
        src_input['rgb_lgt'] = torch.tensor(rgb_lgt)
